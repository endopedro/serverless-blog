...
const AllPosts = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      ignoreRowClick: true,
      cell: row =>
      // <h5 className="delete-element" onClick={()=>deletePost(row._id)}><FontAwesomeIcon icon={faTimesCircle} /></h5>,
      <>
        <h5 className="delete-element" onClick={()=>deletePost(row._id)}>
          <FontAwesomeIcon icon={faTimesCircle} />
        </h5>
        <Link href={`/admin?editPost=${getPostFromId(row._id).slug}`} passHref>
